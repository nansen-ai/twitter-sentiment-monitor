...
from utils import (
    format_number, calculate_cost, truncate_text, sanitize_text,
    save_json, load_json, build_twitter_url, is_spam, calculate_engagement_rate
)
...
def generate_mock_tweets(n: int = 20) -> List[Dict]:
    ...
    tweet_templates = [
        # Positive
        ("Just used @nansen_ai mobile app to catch a whale trade! This is game changing 🚀", "POSITIVE"),
        ("@nansen_ai Season 2 rewards are amazing! Already earned 500 points", "POSITIVE"),
        ("Best trading experience ever with @nansen_ai AI insights", "POSITIVE"),
        ("Nansen Mobile is the future of crypto trading. Highly recommend!", "POSITIVE"),
...
def generate_mock_analyzed_tweets(n: int = 20) -> List[Dict]:
    ...
    sentiment_map = {
        0: 'POSITIVE', 1: 'POSITIVE', 2: 'POSITIVE', 3: 'POSITIVE', 4: 'POSITIVE',
        5: 'NEGATIVE', 6: 'NEGATIVE', 7: 'NEGATIVE', 8: 'NEGATIVE', 9: 'NEGATIVE',
        10: 'NEUTRAL', 11: 'NEUTRAL', 12: 'NEUTRAL', 13: 'NEUTRAL', 14: 'NEUTRAL',
        15: 'NEGATIVE', 16: 'NEGATIVE', 17: 'NEGATIVE', 18: 'POSITIVE', 19: 'MIXED'
    }
    ...
    for i, tweet in enumerate(raw_tweets):
        ...
        analysis = {
            'sentiment': sentiment,
            'confidence': 75 + (i % 20),
            'intent': 'PRAISE' if sentiment == 'POSITIVE' else 'COMPLAINT' if sentiment == 'NEGATIVE' else 'QUESTION',
            'product_mentions': ['nansen_mobile'] if 'mobile' in tweet['text'].lower() else [],
            'themes': ['mobile_adoption'] if sentiment == 'POSITIVE' else ['execution_failures'] if sentiment == 'NEGATIVE' else [],
            'negative_patterns': ['slippage', 'bad_execution'] if 'slippage' in tweet['text'].lower() else [],
            'critical_keywords': ['scam'] if 'scam' in tweet['text'].lower() else ['token'] if 'token' in tweet['text'].lower() else [],
            'urgency': 'HIGH' if 'scam' in tweet['text'].lower() else 'MEDIUM' if sentiment == 'NEGATIVE' else 'LOW',
            'actionable': 'scam' in tweet['text'].lower() or 'crash' in tweet['text'].lower(),
            'summary': tweet['text'][:100],
            'competitive_mentions': ['Arkham'] if 'arkham' in tweet['text'].lower() else [],
            'is_viral': tweet['engagement']['total'] > 100,
            'is_influencer': tweet['is_verified'],
            'strategic_category': 'CRITICAL_FUD' if 'scam' in tweet['text'].lower() else 'STRATEGIC_WIN' if sentiment == 'POSITIVE' and i % 5 == 0 else 'NEUTRAL_MENTION',
...
class TestAggregator(unittest.TestCase):
    ...
    def test_extract_negative_phrases(self):
        ...
        negative_tweets = [t for t in self.mock_tweets if t['analysis']['sentiment'] == 'NEGATIVE']
...
class TestUtils(unittest.TestCase):
    ...
    def test_is_spam_normal_tweet(self):
        ...
        tweet = {
            'text': 'Just used @nansen_ai for trading analysis. Great tool!'
        }
