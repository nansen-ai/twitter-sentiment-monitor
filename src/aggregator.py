...
class SentimentAggregator:
    ...
    def aggregate(self, analyzed_tweets: List[Dict]) -> Dict:
        ...
        positive_tweets = [t for t in analyzed_tweets if t['analysis']['sentiment'] != 'NEGATIVE']
        negative_tweets = [t for t in analyzed_tweets if t['analysis']['sentiment'] == 'NEGATIVE']
        ...
        logger.info(f"Found {positive_count} positive, {negative_count} negative tweets")

        # Validate counts
        if not self._validate_tweet_counts(positive_count, negative_count, total_tweets):
            logger.error(f"Tweet count mismatch! Positive: {positive_count}, Negative: {negative_count}, Total: {total_tweets}")
        ...
        if strategic_highlights['strategic_wins'] > 0 or strategic_highlights['critical_fud'] > 0:
            logger.info(
        ...
        message_1 = self._generate_message_1(
            total_tweets, positive_count, negative_count,
            positive_pct, negative_pct, sentiment_score, trend
        )

        message_2 = self._generate_message_2(
            product_mentions, top_positive_themes, top_negative_themes,
            strategic_highlights, positive_tweets, negative_tweets,
            negative_phrases, positive_theme_groups, negative_theme_groups
        )
            ...
            'positive_themes': [
                {
                    'theme': theme,
                    'count': count,
                    'description': self._format_theme_description(theme, positive_theme_groups[theme]),
                    'example_tweets': [
                        {
            ...
            'negative_themes': [
                {
                    'theme': theme,
                    'count': count,
                    'description': self._format_theme_description(theme, negative_theme_groups[theme]),
                    'example_tweets': [
                        {
                            'url': t['original_tweet']['url'],
                            'username': t['original_tweet']['author_username'],
            ...
            'all_positive_tweets': [
                {
                    ...
                    'username': t['original_tweet']['author_username'],
                    'text': t['original_tweet']['text'],
                    'engagement': t['original_tweet'].get('engagement', {}).get('total', 0)
                }
    ...
    def _generate_message_1(
        self, total: int, positive: int, negative: int,
        pos_pct: float, neg_pct: float, score: float, trend: str
    ) -> str:
...
Overall Sentiment Score: {score:.0f}/100 {trend_emoji}"""
    ...
    def _generate_message_2(
        self, product_mentions: Dict, top_positive: List[Tuple], top_negative: List[Tuple],
        strategic: Dict, positive_tweets: List, negative_tweets: List,
        negative_phrases: List, positive_groups: Dict, negative_groups: Dict
    ) -> str:
        ...
        if top_positive:
            for theme, count in top_positive:
                description = self._format_theme_description(theme, positive_groups.get(theme, []))
        ...
        if top_negative:
            for theme, count in top_negative:
                description = self._format_theme_description(theme, negative_groups.get(theme, []))
        ...
        if any(strategic.values()):
            ...
            if strategic['strategic_wins'] > 0:
                sections.append(f"• {strategic['strategic_wins']} Strategic Wins detected")
            ...
            if strategic['influencer_mentions'] > 0:
                sections.append(f"• {strategic['influencer_mentions']} Influencer Mentions")
    ...
    def _determine_trend(self, current_score: float, historical_scores: List[float]) -> str:
        """
    ...
    def _validate_report(self, report: Dict, tweets: List[Dict]) -> bool:
        ...
        try:
            ...
            # Validate URL format
            for tweet in raw_data['all_positive_tweets'] + raw_data['all_negative_tweets']:
